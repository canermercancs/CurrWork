function [params, y_orig, H_orig, H_clampeds, H_frees, y_frees] = trainHCRF_epoch(curr_epoch, ...
...
%     H_clamped{bag_no}  = doCorrection(Hclamped); % approximation to updateRegionLabels
    ...
    [~, f_hy_clamped(i)]    = potential_hy(yclamped, Hclamped, W_gamma);
    [~, f_hy_free(i)]       = potential_hy(yfree, Hfree, W_gamma);
    [~, f_hh_clamped{i}]    = potential_hh_v2(Hclamped, U, W_alpha);
...
W_alpha     = W_alpha + 0.1*eta .* delta_alpha;
...
% function y = walk_on_y2(num_walks, H, y, params)
%     
%     %%# sample from visible units
%     for k=1:num_walks   
...
% function H = walk_on_H2(num_walks, y, H, X, U, clsf_type, model, params)
% 
%     H = {H};
%     %%# sample from hidden units
%     for k=1:num_walks
